...
function save_topics(callback) {
	...
	chrome.storage.local.set({'listOfTopics': JSON.stringify(topicsArr)}, function(){ $('#myModal').modal('hide');callback()});	
}
...
function save_topic_quick(callback){
	chrome.storage.local.get('listOfTopics', function (result) {
		...
		chrome.storage.local.set({'listOfTopics': JSON.stringify(topics)}, function(){ $('quickAddTopic').modal('hide');if(callback){callback()}});
	});
}
...
function save_home_settings(callback) {
	...
	chrome.storage.local.set({'homeColumns': JSON.stringify(homeColumns)}, function(){ if(callback){callback()}});	
}
...
function remove_topic_column(param_topic, callback){
	chrome.storage.local.get('listOfTopics', function (result) {
		...
		chrome.storage.local.set({'listOfTopics': JSON.stringify(topics)}, function(){ if(callback){callback()}});
	});
}
