...
function getQuoraUserDetails(){
	chrome.alarms.create("QUORA_USER_DETAILS_ALARM", {delayInMinutes:1});
...
function filterByHome(data){
	return _.filter(data, function (item) {return item.type == 'home';});
}
function filterByTopic(data){
	return _.filter(data, function (item) {return item.type == 'topic';});
}
function sortByColIdx(data){
	return _.sortBy(data, function (item) {return item.colIdx;});
}
