...
var apiLoginInWebView = function() {
	...
	webViewHome.on('loadstop', function(e) {
		if(apiLoginCalled != true){
			...
			var injectedJS = "var bodyText = document.body.innerText; "
			+"var isApiLoggedIn; "
			+"var apiResponse = {loggedInUserDetails : '', isApiLoggedIn : ''}; "
			+"bodyText = bodyText.substring(\"while(1);\".length); "
			+"apiResponse.loggedInUserDetails = bodyText; "
			+"if(bodyText == \"\"){apiResponse.isApiLoggedIn = 'false';}else{apiResponse.isApiLoggedIn = 'true';} "		
			+"console.log('bodyText : '+bodyText);callbackData=apiResponse;"; 
			webViewHome.get(0).executeScript({ code: injectedJS }, function(callbackData){
...
var doLogin = function() {
	...
	webViewHome.on('loadstop', function(e) {
		var injectedJS = "var login = document.getElementsByClassName('header_login_text_box'); "
		+"console.log('loaded login' + login.length); var localIsLoggedIn;"
		+"if(login.length > 0 ){console.log('login page'); localIsLoggedIn='false'; } "
		+"else{ console.log('logged-in home page'); localIsLoggedIn='true'; } "
		+"console.log('last line of execute script');callbackData=localIsLoggedIn;"; 
		webViewHome.get(0).executeScript({ code: injectedJS }, function(callbackData){
			...
			if(callbackData == "true"){
				...
				chrome.storage.local.set({'isQDInitialized': 'true'}, function(){
					restore_topics();
...
var callApiToGetUserDetails = function() {
	...
	userDetailsWebView.on('loadstop', function(e) {
		if(API_CALLED_TO_GET_USER_DETAILS != true){
			...
			+"if(bodyText == \"\"){apiResponse.isApiLoggedIn = 'false';}else{apiResponse.isApiLoggedIn = 'true';} "		
			+"console.log('bodyText : '+bodyText);callbackData=apiResponse;"; 
			userDetailsWebView.get(0).executeScript({ code: injectedJS }, function(callbackData){
				var callbackResponse = callbackData[0];
...
var matrix_reloaded = function() {
	...
	chrome.storage.local.get('listOfTopics', function (result) {
		if(result && (result.listOfTopics === undefined)){
			...
			chrome.storage.local.set({'listOfTopics': JSON.stringify(topics)}, function(){ display_topics(topics)});
		}
		else{
			//add columns
			...
			chrome.storage.local.get('homeColumns', function (result) {
				if(result && (result.homeColumns === undefined)){
					...
					chrome.storage.local.set({'homeColumns': JSON.stringify(homeColumns)}, function(){ display_home_settings(homeColumns)});
				}
				else{
						...
						if(i ==0 || topics[i] != ""){
							...
							(function (navBtn, i) {
								navBtn.addEventListener('click', function(){
									var leftPos = 522 * i;
									$('html, body').animate({scrollLeft: leftPos}, 800);
								});
							})(navBtn, i);
